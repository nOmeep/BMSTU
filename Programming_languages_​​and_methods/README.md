...
- <font color=green>Поток</font> - реализация интерфейса `Stream<T>`

<u>Преобразование последовательностей</u>:

1. Методы интерфейса `Stream<T>` позволяют преобразовывать последовательность:
    
    - **Фильтрация**: формирование потока, содержащего только те элементы исходного потока, которые удовлетворяют предикату
    ```Java
    Stream<T> filter(Predicate<? Super T> predicates)
    ```

    - Формирование потока, представляющего последовательность значений, полученную путем вызова функции для каждого элемента потока
    ```Java
    <R> Stream<R> map(Function<? super T, ? extends R> mapper)
    ```

    - **Конкатенация потока**: конкатенация потоков, полученных путем вызова функции для каждого элемента исходного потока
    ```Java
    <R> Stream<R> flatmap(Function <? super T, 
        ? extends Stream<? extends R>> mapper)
    ```

<u>**Пример использования преобразований `filter` и `map`**</u>:

<u>Задача</u>:

- Пусть дан динамический массив `ArrayList<Integer>`. Необходимо вывести в стандартный поток вывода последовательность чисел, получаемую путем возведения числа 2 в степени, заданные числами из этого массива. При этом отрицательные степени должны пропускаться

```Java
public static void powers(ArrayList<Integer> list) {
    list.stream().filters(x -> x >= 0).map(x -> 1 << x)
    .forEach(System.out::println);
}
```

<u>Пример использования `flatMap`</u>:

Предположим есть матрица...

```Java
public static void nonZeroes(ArrayList<Integer> m) {
    m.stream()
...
### Собирание последовательностей

Потребление значений из потоков выполняется так называемыми **коллекторами** - объекты классов, реализующие интерфейс `Collector`.

Коллекторы предназначены для выполнения анализа потоков и записи результатов реализуемых потоками преобразований в объекты контейнерных классов.

*Для задействования некоторого коллектора необходимо вызвать метод `collect` потока*

```Java
<R, A> R collect(Collector<? super T, A, B> collector)
```

## Механизм пакетов

Начиная с определенного размера программы, для ее структурирования начинает не хватать декомпозиции только на классы, требуется механизм пакетов для группировки классов:

- образуется группа взаимосвязанных классов, которые целезообразно выделить в отдельные подсистемы и ограничить доступность некоторых классов рамками подсистемы;

- возникает потребность упростить навигацию по исходным кодам программы для облегчения поиска исходных кодов, которые относятся к той или иной подсистеме;

- появляются сложности присвоения уникальных имен классов(особенно в случае командной разработки);

<u>**Определение**</u>:

- <font color=green>**Пакет**</font> - контейнер классов, предоставляющий для них отдельное пространство имен  дополнительные возможности по управлению доступом

### Имена пакетов и квалифицированные имена классов

<u>**Определение**</u>:

- <font color=green>**Имя пакета**</font> - непустая последовательность идентификаторов, разделенная точкой

<u>**Замечание**</u>: в именах пакетов, как правило, не используются заглавные символы, если это не аббревиатура. Также допускаются символы подчеркивания

<u>*Примеры*</u>:

```Java
java.util.coucurrentorg.omg.CORBA_1_10.portable.ru.bmstu.ing // example
```

<u>**Определение**</u>:

- Квалифицированное имя класса - это имя, с помошью которого можно обращаться к публичному классу извне пакета, которому этот класс принадлежит
...
```Java
java.util.coucurrent.CoucurrentLinkedQueue

// coucurrent - пакет 
// CoucurrentLinkedQueue - неограниченная очередь(что-то такое, неважно)
```

### Создание пакета 

Для создания пакета небходимо поместить в начало каждого файла, содержащего исходный текст классов, директиву:
...
- классы, объявленные в файлах, в которых отсутствует директива `package` считаются принадлежащими к безымянному пакету - `default package`

- компилятор `Java` требует, чтобы файлы пакета размещались в файловой системе в каталоге, путь которого относительно текущего каталога(в котором запущен компилятор) соответсвовал имени пакета

<u>*Пример*</u>:

Файлы пакета `ru.bmstu.iu9` должны лежать в каталоге `ru/bmstu/iu9`

<u>**Замечание**</u>:

- Файлы, принадлежащие дефолтному пакету должны лежать в текущем каталоге

<u>**Замечание**</u>:

- Один java-файл может содержать не более одного публичного класса и любое количество непубличных классов

- **Публичные классы можно ипользовать как внутри, так и снаружи** пакета, **непубличные доступны внутри, а извне не видны**

- К членам классов пакета, объявленным без модификатора доступа, можно обращаться из любого места внутри пакета, но запрещено обращаться снаружи

### Импорт классов из другого пакета

<u>**Утверждение**</u>

- **Чтобы обратиться к классу, принадлежащему другому пакету, этот класс необходимо импортировать**

<u>ТРИ СПОСОБА ИМПОРТА КЛАССОВ</u>

1.  Использование квалифицированного имени:
    ```Java
    java.util.HashSet<String> a = new java.util.HashSet<>();
    ```

1. Указание в директиве `import`:
    ```Java
...
1. Использовать директивы `import` для импорта всех классов пакета:
    ```Java
    import java.util.*; // начало файла; * - импорт всех имен
...
Для упрощения доступа к статическим членам классов существует специальная форма директивы `import`:

```Java
import static имя_пакета.имя_класса.имя_члена_класса;
...
<u>*Пример(без импорта и с импортом)*</u>:

1. Пример без импорта статических членов класса `Math`

```Java
double l = Math.cos(Math.PI/2);
```

2. Пример с импортом статических членов класса `Math`
...
<u>*Пример не корректного приведение типов*</u>:
...
// динамическое приведение невозможно, выдаст ошибку
```

<u>Пример правильного динамического приведения типов</u>:
...
```C++
class Y : mod x1, mod x2, ..., mod xn
{
    ...
}
```

`mod` - модификатор(`public`, ...)
...
int main() {
    C c;
    c.m(); // компилятор g++ выдаст ошибку!
    return 0;
}
```

Для разрешения противоречий в именах при множественном наследовании нужно использовать **квалифицированные имена**!
...
{
    C c;
    c.A::m(); // квалифицированное имя, поэтому ОК
    return 0;
}
```
...
- <font color=green>**Иерархия наследования**</font> - ориентированный ациклический граф, множеством узлов которого является множество классов программы

<u>**Замечание**</u>:

- Если класс `Y` является непосредственным базовым классом для класса `X`, то из узла `X` исходит дуга в `Y`

- Класс `B` является классом противоречия, если существует класс `A`, что в иерархии наследования можно провести не менее 2х непересекающихся путей из `A` в `B`

- Иерархия наследвания называется противоречивой, если в ней существуют классы противоречия

## Проблемы противоречивых иерархий

**Утверждение**:

- Основной проблемой противоречивых иерархий является возможность многократного включения полей базового класса производным
...
- <font color=green>**Виртуальное наследование**</font> - способ реализации наследования, гарантирующий, что базовый класс не будет включен ни в один из производных классов более чем в одном экземпляре.

- Если класс `B` виртуально наследует классу `A`, то говорят, что `A` - виртуальный базовый класс для класса `B`

```C++
class B : mod virtual A // "mod" - public, potected, private
{
    ...
};
```

<u>**Пример виртуального наследования**</u>:
...
- <font color=green>**Формальные параметры шаблона**</font> - это идентификаторы, областью видимости которых является тело шаблона. Они могут обозначать типы, зачения или другие шаблоны. При применении шаблона формальные параметры в его теле заменяются на конкретные типы, значения или шаблоны, и полученный код компилируется

<u>**Пример шаблона функции**</u>:
...
- Нетиповые параметры шалона представляют **значения, а не типы**:
    - Их синтаксис похож на объявление переменных
    - Внутри тела шаблона имя такого параметра обозначает константу указанного типа

<u>**Пример шаблона для матриц**</u>:
...
```C++
template<template<typename T, int, int> class M, typename T, int P, int Q, int R>
void multuply(M<T, P, R>& c, M<T, P, Q>& a, M<T, Q, R>& b)
{
    M<T, P, R> res;
    for(int i = 0; i < P; i++) {
        for (int j = 0; j < R; j++) {
...
- <font color=green>**Зависимый идентификатор в шаблоне**</font> - любое имя в шаблоне, которое зависит от формальных парметров шаблона.

<u>**Замечание**</u>:

- Смысл зависимых идентификаторов становится известным компилятору только при инстациации шаблона, когда известны его фактические параметры
...
- <font color=green>**Инстанциация шаблона**</font> - это порождение кода по шаблону и списку фактических параметров

```C++
имя_шаблона <список_фактических_параметров>
```
...
```C++
Matrix<int 4, 5> m;
Matrix<typename, int, int>

//

do_swap<Matrix<int, 4, 5>> (m, что_тут_не_понял)
```
